...
class MisteryClassifier:
    ...
    def fit(self,X,y):
        ...
        for m in range(self.nWL):
            ...
            #print "w_learn=%s" % (w_learn)
            norm_w = np.exp(w)
            #print "norm_w = %s" % (norm_w)
            
            norm_w /= norm_w.sum()
            #print "norm_w_new = %s" % (norm_w)
...
#ax.set_xticklabels([])
pl.savefig('E:\\Purdue Courses\\Second semester\\CS573 Data Mining\\CS573 homework\\HW5\\1.png', dpi = 1000)
pl.show()
